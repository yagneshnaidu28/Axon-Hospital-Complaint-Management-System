...
const submitBtn = document
  ...
  .addEventListener("click", function (e) {
    ...
    if (
      ...
      if (inputData.storage === "Cleanliness") {
        let studentsLocal = JSON.parse(Cleanliness.getItem("student"));
        if (studentsLocal === null) {
          studentsLocal = [];
          studentsLocal.push(inputData);
          Cleanliness.setItem("student", JSON.stringify(studentsLocal));
        } else {
          studentsLocal.push(inputData);
          Cleanliness.setItem("student", JSON.stringify(studentsLocal));
        }
      } else if (inputData.storage === "Accounting_mistake") {
        let studentsSession = JSON.parse(Accounting_mistake.getItem("student"));
        if (studentsSession === null) {
          studentsSession = [];
          studentsSession.push(inputData);
          Accounting_mistake.setItem("student", JSON.stringify(studentsSession));
        } else {
          studentsSession.push(inputData);
          Accounting_mistake.setItem("student", JSON.stringify(studentsSession));
        }
      } else {
        let studentsCookie = Doctor/NurseMismanagement.cookie;
        if (studentsCookie === "") {
          studentsCookie = [];
          studentsCookie.push(inputData);
          Doctor/Nurse_Mismanagement = `student = ${JSON.stringify(studentsCookie)}`;
        } else {
          studentsCookie = studentsCookie.split("=");
          studentsCookie = JSON.parse(studentsCookie[1]);
          studentsCookie.push(inputData);
          Doctor/Nurse_Mismanagement.cookie = `student = ${JSON.stringify(studentsCookie)}`;
        }
      }
    } else {
